...
function RetakeQuestionList({ questions, navigate, onError }) {
  if (questions.length === 0) {
    return <p className="text-sm text-slate-400">Nothing here right now.</p>;
  }
  return (
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      apiFetch('/analytics/hardest-questions'),
      apiFetch('/analytics/review-queue'),
    ])
      .then(([s, h, hardest, review]) => {
        setSummary(s);
        setHistory(h.attempts);
        setHardestQuestions(hardest.questions);
        setReviewQueue(review.questions);
      })
  ...
  if (!summary || !history || !hardestQuestions || !reviewQueue) {
    return <LoadingIndicator label="Loading dashboard..." fullPage />;
  }
